...
export const CustomerSidebar: React.FC<CustomerSidebarProps> = ({
  ...
  const { saasAccess, subscription } = useCustomer();
    ...
    <aside className="w-64 bg-slate-900 text-white flex flex-col h-full border-r border-slate-800">
      ...
      <div className="p-4 border-t border-slate-800 space-y-3 bg-slate-950/40">
        ...
        <button
          type="button"
          onClick={() => handleNav('/')}
          className="w-full py-2 px-3 text-slate-400 hover:text-slate-200 text-xs font-medium rounded-lg hover:bg-slate-800/50 transition-colors flex items-center gap-2"
        >
